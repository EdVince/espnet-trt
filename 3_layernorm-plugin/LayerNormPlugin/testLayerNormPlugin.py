...
def layerNormCPU(bufferH, epsilon):
    _x = bufferH[0]
    _0 = np.mean(_x.reshape(_x.shape[0], -1), 1)[:, np.newaxis, np.newaxis]
    _1 = _x - _0
    _2 = _1 * _1
    _3 = np.mean(_2.reshape(_x.shape[0], -1), 1)[:, np.newaxis, np.newaxis]
    _4 = _3 + epsilon
    _5 = np.sqrt(_4)
    _6 = 1 / _5  # 1/sqrt(...)
    _7 = _1 * _6  # (x-μ)/sqrt(...)
...
if __name__ == '__main__':
    ...
    if os.path.isfile(trtFile):
        ...
        inputTensorList.append(network.add_input('inputT', trtDataType, [-1, -1, in2]))

        profile = builder.create_optimization_profile()
        profile.set_shape('inputT', [1, 1, in2], [in0, in1, in2], [2*in0, 2*in1, in2])
    ...
    context.set_binding_shape(0, [in0, in1, in2])
    ...
    data = np.random.rand(in0, in1, in2).astype(np.float32)
    # data = np.arange(in0 * in1 * in2).reshape(in0, in1, in2).astype(npDataType)
